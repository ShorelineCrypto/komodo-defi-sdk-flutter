...
This package fetches the unified coins configuration JSON from the `ShorelineCrypto/coins` repository (`utils/coins_config_unfiltered.json` by default), converts entries into `Asset` models (from `komodo_defi_types`), persists them to Hive, and tracks the source commit so you can decide when to refresh.

## Features

- Fetch latest commit from the `ShorelineCrypto/coins` repo
...
final config = AssetRuntimeUpdateConfig(
  ...
  coinsRepoApiUrl: 'https://api.github.com/repos/ShorelineCrypto/coins',
  coinsRepoContentUrl: 'https://raw.githubusercontent.com/ShorelineCrypto/coins',
